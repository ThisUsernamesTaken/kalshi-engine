...
async def _run_loop(kalshi_ws, state, alpaca, log, duration_s: float) -> None:
    ...
    async def pump():
        # Phase 14.17 - restart the feed iterator on any unexpected error so a
        # single bad frame (e.g. the OverflowError seen 2026-05-27) can't leave
        # the observer permanently deaf. Malformed frames are already skipped
        # inside KalshiWebSocketFeed; this is the outer safety net.
        while True:
            try:
